...
interface BillingProps {
  ...
  onCompleteTransaction: (items: CartItem[], total: number, customerId: string | null, paymentMethod: PaymentMethod) => void;
...
const Billing: React.FC<BillingProps> = ({ products, merchantProfile, onCompleteTransaction, onAddCustomer }) => {
  ...
  const handleCheckout = () => {
    ...
    onCompleteTransaction(cart, totalAmount, customerId, paymentMethod);
    setCart([]);
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 relative">
      ...
      {/* Bottom Sheet Total */}
      {cart.length > 0 && (
        <div className="p-4 border-t bg-white shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] sticky bottom-0 z-10 rounded-t-2xl">
            <div className="flex justify-between items-center mb-4">
                <span className="text-slate-500 text-sm font-medium">{cart.length} Items</span>
                <div className="text-right">
                    <span className="text-xs text-slate-400 block font-bold uppercase">Total to Pay</span>
                    <span className="text-2xl font-bold text-slate-900">₹{totalAmount}</span>
                </div>
            </div>
            <button 
                onClick={() => setShowCheckout(true)}
                className="w-full bg-slate-900 text-white py-4 rounded-xl font-bold text-lg flex items-center justify-center gap-2 active:scale-95 transition-transform"
            >
      ...
      {/* Checkout Modal */}
      {showCheckout && (
        <div className="fixed inset-0 bg-white z-50 flex flex-col animate-slide-up overflow-hidden">
            ...
            <div className="p-4 border-t bg-white sticky bottom-0">
                <button 
                    ...
                    className="w-full bg-emerald-600 text-white py-4 rounded-xl font-bold text-lg shadow-lg shadow-emerald-200 active:scale-95 transition-transform disabled:opacity-50 disabled:scale-100"
                >
                    Confirm & Print Bill
                </button>
            </div>
